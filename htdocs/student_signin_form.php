			...
			jQuery("#signin_student").submit(function(e){
					...
					$.ajax({
						...
						if(html=='true')
						{
						$.jGrowl("Welcome to Course Management System", { header: 'Sign up Success' });
						var delay = 2000;
							setTimeout(function(){ window.location = 'dashboard_student.php'  }, delay);  
						}else if(html=='false'){
							$.jGrowl("Student is not present in the database. Please Check Your ID, Firstname, Lastname and the Section You Belong. ", { header: 'Sign Up Failed' });
						}
						}
						...
						{
						$.jGrowl("Student is not present in the database", { header: 'Sign Up Failed' });
						}
				});
			});
			</script>
			...
			<a onclick="window.location='index.php'" id="btn_login" name="login" class="btn btn-success" type="submit"><i class="icon-signin icon-large"></i> Back to Login Page</a>
